...
class Memory(nn.Module):
  ...
  def __init__(self, n_nodes, memory_dimension, input_dimension, message_dimension=None,
               device="cpu", combination_method='sum', seq_len=0):
  ...
  def __init_memory__(self):
    ...
    if self.seq_len > 0:
      self.message_buffer = nn.Parameter(torch.zeros((self.n_nodes, self.seq_len, self.message_dimension)).to(self.device),
                                       requires_grad=False)
      self.buffer_lengths = nn.Parameter(torch.zeros(self.n_nodes, dtype=torch.long).to(self.device),
                                         requires_grad=False)
  ...
  def backup_memory(self):
    ...
    if self.seq_len > 0:
      return self.memory.data.clone(), self.last_update.data.clone(), messages_clone, self.message_buffer.data.clone(), self.buffer_lengths.data.clone()
  ...
  def restore_memory(self, memory_backup):
    ...
    if self.seq_len > 0:
      self.message_buffer.data, self.buffer_lengths.data = memory_backup[3].clone(), memory_backup[4].clone()

  def detach_memory(self):
    self.memory.detach_()
    if self.seq_len > 0:
      self.message_buffer.detach_()
      self.buffer_lengths.detach_()
