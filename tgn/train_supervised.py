...
if DATA.startswith("tgbl"):
  ...
  full_data, node_features, edge_features, train_data, val_data, test_data, _, _ = \
    get_tgb_data(DATA, use_validation=args.use_validation)
else:
...
for i in range(args.n_runs):
  ...
  for epoch in range(args.n_epoch):
    ...
    val_auc, val_f1, val_lat = eval_node_classification(tgn, decoder, val_data, full_data.edge_idxs, BATCH_SIZE,
    ...
    logger.info(f'Epoch {epoch}: train loss: {loss / num_batch}, val auc: {val_auc}, val f1: {val_f1}, val lat: {val_lat}, time: {time.time() - start_epoch}')
  ...
  if args.use_validation:
    ...
    test_auc, test_f1, test_lat = eval_node_classification(tgn, decoder, test_data, full_data.edge_idxs, BATCH_SIZE,\
                                         n_neighbors=NUM_NEIGHBORS)
  else:
  ...
  logger.info(f'test auc: {test_auc}, test f1: {test_f1}, test lat: {test_lat}')